	...
	public class KrakenHypnosisVortex : ModProjectile
	{
		...
		public override void AI() {
			...
			for (int i = 0; i < Main.maxPlayers; i++) {
				...
				toCenter.Normalize();
				float pull = MathHelper.Lerp(1.2f, 0.18f, distance / 1240f);
				player.velocity += toCenter * pull;
				player.AddBuff(BuffID.Slow, 2);

				if (distance < 86f && !player.immune) {
					player.Hurt(PlayerDeathReason.ByCustomReason(NetworkText.FromLiteral(player.name + " was pulled into the alien core.")), 9999, player.Center.X < Projectile.Center.X ? -1 : 1);
				}
			}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.spriteBatch.Draw(texture, center, null, new Color(15, 35, 96, (byte)(130 * opacity)), Projectile.rotation, origin, 1.72f * pulse, SpriteEffects.None, 0f);
			Main.spriteBatch.Draw(texture, center, null, new Color(76, 162, 255, (byte)(72 * opacity)), -Projectile.rotation * 0.72f, origin, 1.38f, SpriteEffects.FlipHorizontally, 0f);
			return false;
		}
