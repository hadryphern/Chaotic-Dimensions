...
namespace ChaoticDimensions.Common.Systems
{
	public class KrakenEventSystem : ModSystem
	{
		...
		public override void PostDrawInterface(SpriteBatch spriteBatch) {
			...
			DrawCombatGrade(spriteBatch, pixel);
			DrawAlienFog(spriteBatch);
			DrawLightningFlash(spriteBatch, pixel);
			DrawAlienVisionMask(spriteBatch);
			DrawInk(spriteBatch);
		}
		...
		private float GetRainIntensity() {
			...
			return MathHelper.Lerp(0.28f, 0.98f, Utils.GetLerpValue(SecondCameraStart, BattleStart, Timer, true));
		}
		...
		private void DrawLightningFlash(SpriteBatch spriteBatch, Texture2D pixel) {
			...
			spriteBatch.Draw(pixel, new Rectangle(-8, -8, Main.screenWidth + 16, Main.screenHeight + 16), new Color(130, 160, 210, (byte)(6 * flash)));
		}
		...
		private void DrawAlienFog(SpriteBatch spriteBatch) {
			...
			Color near = Color.Lerp(new Color(78, 84, 104, 22), new Color(116, 48, 60, 25), phase2Rage);
			Color far = Color.Lerp(new Color(24, 27, 42, 28), new Color(50, 12, 20, 30), phase2Rage);
		...
		private void DrawCombatGrade(SpriteBatch spriteBatch, Texture2D pixel) {
			Color deepGrade = Color.Lerp(new Color(1, 8, 34, 54), new Color(28, 4, 10, 48), phase2Rage);
			Color accentGrade = Color.Lerp(new Color(0, 28, 78, 20), new Color(92, 12, 20, 16), phase2Rage);
			spriteBatch.Draw(
				pixel,
			...
			if (phase2TransitionTimer > 0) {
				float progress = 1f - phase2TransitionTimer / 180f;
				float pulse = (float)System.Math.Sin(progress * MathHelper.Pi);
				spriteBatch.Draw(
					pixel,
					new Rectangle(-8, -8, Main.screenWidth + 16, Main.screenHeight + 16),
					new Color(124, 8, 18) * (pulse * 0.1f)
				);
			}
		}
		...
		private void DrawAlienVisionMask(SpriteBatch spriteBatch) {
			...
			spriteBatch.Draw(mask, center, null, Color.White, 0f, origin, scale, SpriteEffects.None, 0f);
		}
		...
		private void DrawSpaceHaze(SpriteBatch spriteBatch) {
			...
			Vector2 center = new Vector2(Main.screenWidth * 0.5f, Main.screenHeight * 0.48f);
			Color haze = Color.Lerp(new Color(38, 42, 62, 24), new Color(86, 28, 40, 25), phase2Rage);
